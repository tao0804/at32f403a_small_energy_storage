...
{
  // 能被4整除且不能被100整除的年份是闰年;如果一个年份能被4被100整除,还被400整除，是闰年,否则是平年
  if(year % 4 == 0)
  {
    if(year % 100 == 0)
    {
...
uint8_t rtc_time_set(calendar_type *calendar)
{
  ...
  if(calendar->year < 2000 || calendar->year > 2199)
  {
    return 1;
  }

  for(t = 2000; t < calendar->year; t++)
  {
    if(is_leap_year(t))
    {
      seccount += 31622400;	// 366
    }
    else
    {
      seccount += 31536000;	// 365
    }
  }
  ...
  {
    seccount += (uint8_t)mon_table[t] * 86400;	// 86400:1天
